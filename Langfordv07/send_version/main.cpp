...
int main(int argc, char *argv[]) {
  ...
  if (nb_tache_total % p == 0) {
    nb_tache =
        nb_tache_total / p;   // nombre des tâches pour tous les autres process
    nb_tache_last = nb_tache; // nombre de tâches du dernier process
  } else {
    nb_tache = (int)(nb_tache_total /
                     (p - 1)); // nombre des tâches pour tous les autres process
    nb_tache_last =
        nb_tache_total % (p - 1); // nombre de tâches du dernier process
  }
  ...
  if (id != p - 1) {
    ...
    solution_local_flat.resize(nb_tache_last *
                               N); // Tableau de solutions local pour le send
  ...
  if (id == p - 1) {
    ...
    for (k = 0; k < (p - 1); k++) {
      ...
      if (k < p - 1) {
        MPI_Send(&solution_global_flat[k * nb_tache * N], nb_tache * N, MPI_INT,
                 k, 0, MPI_COMM_WORLD);
      }
    }

    copy(solution_global_flat.end() - nb_tache_last * N,
         solution_global_flat.end(), solution_local_flat.begin());

  } else { // reception des taches des process de 0 à p-2
    MPI_Recv(solution_local_flat.data(), nb_tache * N, MPI_INT, p - 1, 0,
             MPI_COMM_WORLD, 0);
  }
