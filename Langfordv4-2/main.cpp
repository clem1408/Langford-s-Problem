...
void generateCombinations(int depth, const vector<int> &max_pos_tab,
                        int &count,
...
inline void remove_pair(vector<int> &langford,
...
inline int place_pair(vector<int> &langford,
...
void init_general_tab(vector<int> &langford,
                    vector<int> &general_tab) {
for (int i = N - DEPTH; i < N; i++) {
...
void langford_algorithm(vector<int> &langford,
                        const vector<int> &max_pos_tab, int &local_count) {
int level = N - DEPTH;
vector<int> general_tab(2 * N, 0);
