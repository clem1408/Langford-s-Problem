...
void generateCombinations(int depth, const std::vector<int>& max_pos_tab, int& count, std::vector<std::vector<int>>& solutions) {
    ...
    while (true) {
        ...
        for (int i = 0; i < depth && isValid; ++i) {
            int value = indices[i];
            int pairPos1 = N - depth + i;
            int pairPos2 = pairPos1 + value + 1;

            for (int j = i + 1; j < depth; ++j) {
                int otherValue = indices[j];
                int otherPairPos1 = N - depth + j;
                int otherPairPos2 = otherPairPos1 + otherValue + 1;

                if (pairPos1 == otherPairPos1 || pairPos1 == otherPairPos2 ||
                    pairPos2 == otherPairPos1 || pairPos2 == otherPairPos2) {
                    isValid = false; // Chevauchement détecté
                    break;
                }
            }
        }

        // Si la solution est valide et non triviale, l'ajouter aux solutions
        if (isValid) {
